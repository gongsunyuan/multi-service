...
def train_ppo() -> None:
    ...
    if args.checkpoint:
        logger.log(f"Loading PPO checkpoint from: {args.checkpoint}", tag="Checkpoint")
        try:
            agent.load_state_dict(torch.load(args.checkpoint, map_location=config.device))
            logger.log(f"PPO checkpoint loaded successfully!")
        except Exception as e:
            logger.log(f"Failed to load PPO checkpoint: {e}", tag="Checkpoint Warn")
        ...
        if hasattr(config.path, 'warmup_path') and config.path.warmup_path:
            logger.log(f"Loading warmup pre-trained model from: {config.path.warmup_path}", tag="Warmup")
            warmup_info = trainer.checkpoint_manager.load(
                model=agent,
                checkpoint_path=config.path.warmup_path,
                device=config.device
