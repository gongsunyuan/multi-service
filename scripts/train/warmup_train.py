...
def train_stage1(config: AttrDict) -> None:
    ...
    # 3. 初始化 TensorBoard
    writer = SummaryWriter(log_dir=f"runs/stage1_warmup_{config.experiment_name}")
    ...
    for epoch in range(config.train.epochs):
        ...
        if loss < best_loss:
            best_loss = loss
            torch.save(agent.state_dict(), os.path.join(save_path, "warmup_best.pt"))
            logger.log(f"Saved Best Model at Epoch {epoch+1}")
            
    # 保存最终权重
    torch.save(agent.state_dict(), os.path.join(save_path, "warmup_final.pt"))
