...
def consolidate_project(source_dir, output_filename):
    ...
    """
    # 定义排除列表
    exclude_dirs = ['.git', '__pycache__', '.idea', '.vscode', 'venv', 'build', 'train_log', 'tmp', 'trained_model', 'data', 'Test', 'Records', 'workspace']
    exclude_extensions = ['.pyc', '.png', '.jpg', '.jpeg', '.gif', '.exe', '.bin', '.pkl', '.pth', 'igest', 'log', 'ingest']
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="合并项目目录树和代码内容")
    parser.add_argument("--target_dir", type=str, default="./src", help="目标项目目录路径")
    parser.add_argument("--output_file", type=str, default="agent_code.txt", help="输出文件路径")
    
    args = parser.parse_args()
    os.makedirs("workspace/outputs/code_files", exist_ok=True)
    # 设置目标路径
    target_directory = args.target_dir
    output_file = os.path.join("workspace/outputs/code_files", args.output_file)
