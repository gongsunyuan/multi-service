...
class FiLMPPOAgent(nn.Module):
  ...
  def get_action(self, state, node_embeds, curr_node_idx, target_node_idx, 
    ...
    Args:
      ...
      neighbor_indices: 有效邻居 ID 列表 (List[int]) - 已剔除回头路
    ...
    mask = state.action_mask[neighbor_indices]  # (K,)
    logits[~mask] = -1e9  # 屏蔽非法动作
