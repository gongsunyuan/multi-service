...
from multi_service.models import actor, critic

class DrlAgent(BaseSDNAgent):
    """
    DRL Agent: Uses Linear projection for node encoding (No GNN, No FiLM).
    """
    def __init__(self, 
                 node_dim, 
                 edge_dim, 
        ...
        # 1. Initialize Core Encoder (Linear only)
        # DRL mode: Linear layer maps original node dimension to hidden_dim 
        self.encoder = nn.Linear(node_dim, hidden_dim)
        
        logger.debug("DrlAgent initialized with Linear encoder (no GNN, no FiLM)")
    ...
    def get_node_embeddings(self, graph_data, fingerprint):
        """
        Uses Linear projection for node features.
        """
        # Linear projection
        h = self.encoder(graph_data.x) # Expected (N, H)
        
        # Check if dimension was squeezed
        if h.dim() == 1:
            h = h.unsqueeze(0)
