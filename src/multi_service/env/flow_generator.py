...
class FlowGenerator:
    ...
    def generate_traffic_matrix(self, nodes, G_nx, total_load_mbps=500.0):
        ...
        if len(tm) > MAX_BG_FLOWS:
            ...
            logger.info(f"Optimized: kept top {MAX_BG_FLOWS} flows. Max flow: {top_flows[0][1]:.2f} Mbps")
            return final_tm
    ...
    def apply_traffic_matrix_to_mininet(self, net, tm_dict, G_nx, install_rules_func, duration=1000):
        ...
        """
        logger.info(f"Injecting {len(tm_dict)} background flows (Ghost Strategy)...")
        logger.debug(f"Duration set to: {duration} seconds (Ensure this > target flow time!)")
        ...
        # ==========================================
        logger.debug("Starting ITGRecv daemons on all hosts...")
        for h in net.hosts:
        # 建议记录 Recv 日志以便排查控制平面问题
            h.popen(f"nice -n 2 ITGRecv > /tmp/itgrecv_{h.name}.log 2>&1 &")
        
        time.sleep(1)
        # ==========================================
        # Phase 1: 铺设路径 & 设置陷阱
        # ==========================================
        logger.debug("Installing routing rules & Ghost drop policies...")
