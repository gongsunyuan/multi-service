...
class PPOTrainer:
    def __init__(self, agent: Any, env: SdnWrapper, memory: PPOMemory, config: DictConfig | ListConfig) -> None:    
    ...
    def train_one_epoch(self, traffic_generator: BankTrafficManager):
        ...
        logger.info(f"Starting rollout for {len(flows)} flows...")
        for flow in flows:
            ...
            logger.info(f"[Flow {flow_count+1:02d}] {flow.flow_type.name} Task: Node {flow.src} -> Node {flow.dst}")  
            ...
            while not done and step_count < self.config.train.max_steps:
                with torch.no_grad():
                    ...
                    if next_node is None:
                        logger.debug(f"Dead path{self.env.path_so_far}, ai failed to find path !")
                    ...
                    #  Clean Step Logging
                    logger.debug(f"Step {len(self.env.path_so_far)-1:02d}|{curr_node} -> {next_node} | Reward: {reward:.4f}")
            ...
            # 打印的是 episode_reward
            logger.debug(f"[Flow {flow_count:02d}] Finished | Total Reward: {episode_reward:.2f} | Path: {'->'.join(map(str, self.env.path_so_far))}")
        ...
        if len(self.memory.rewards) > 0:
            ...
            logger.debug("No valid transitions collected, skipping agent update")
    ...
    def run(self, traffic_generator):
        ...
        主训练入口
        """
        logger.info(f"Start training for {self.config.train.max_epochs} epochs...")
        ...
        for epoch in range(self.start_epoch, self.config.train.max_epochs):
            ...
            if use_curriculum:
                ...
                OmegaConf.update(self.config, "load_flow", current_load, force_add=True)
            ...
            logger.info(f"Epoch {epoch}: Reward={train_reward:.4f} | Current LR={self.agent.get_current_lr():.8f} |Time={duration:.2f}s")
