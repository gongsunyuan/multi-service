...
class PPOTrainer:
  ...
  def train_one_epoch(self, traffic_generator):
    ...
    for flow in flows:
      ...
      step_count = 0
      episode_reward = 0 # [修复 3] 单条流的累积奖励
      ...
      while not done and step_count < self.config.train.max_steps:
        with torch.no_grad():
          ...
          if len(valid_local_indices) == 0:
            ...
            # 现在 val 已经定义了，可以安全存储
            logger.log("Dead path, ai failed to find path !", tag="Path Failed")
      ...
      # 打印的是 episode_reward
      logger.log(f"Flow {flow_count} finished. Reward: {episode_reward:.2f}", tag="Flow", log_to_console=True)
  ...
  def save_checkpoint(self, epoch):
    ...
    ckpt_path = os.path.join(self.config.path.ckpt_dir, f"checkpoint_{epoch}.pth") # 
    os.makedirs(self.config.path.ckpt_dir, exist_ok=True) # 
    ...
    # 获取目录下所有的 .pth 文件
    checkpoint_files = glob.glob(os.path.join(self.config.path.ckpt_dir, "*.pth"))
    ...
    if len(checkpoint_files) > 2:
      ...
      files_to_delete = checkpoint_files[:-self.config.train.checkpoint_keep_count]
