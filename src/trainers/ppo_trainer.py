...
class PPOTrainer:
  def __init__(self, agent: FiLMPPOAgent, env: SdnWrapper, memory: PPOMemory, config: AttrDict) -> None:    
        ...
        while not done and step_count < self.config.train.max_steps:
            with torch.no_grad():
                ...
                if next_node is None:
                    ...
                    self.memory.store(
                        state=state,            
                        action=torch.tensor(-1, device=self.agent.device),  # 使用-1表示无效action  
