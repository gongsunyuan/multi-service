...
class VerboseLogger:
  ...
  def configure(self, log_file=None, verbose=True, console=True):
    """[初始化入口] 在程序启动时配置一次。"""
  ...
  def log(self, message, tag=None, timestamp=None, log_to_console=None, **kwargs):
    """核心打印函数 (替代 vprint)。"""
  ...
  def log_matrix(self, tensor: torch.Tensor, filename: str, flow_id: int):
    """
    [整合] 将流量指纹矩阵写入文件。
  ...
  def log_qos(self, service_type, delay, jitter, bw, loss, tag="Measure"):
    """QoS 报告格式化。"""
    if not self.enabled: return
